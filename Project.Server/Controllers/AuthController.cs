...
public class AuthController : ControllerBase
{
    ...
    private readonly MicrosoftTokenValidator _msValidator;
    ...
    public AuthController(
        ...
        MicrosoftTokenValidator msValidator)
    {
        _db = db;
        _ad = ad;
        _jwt = jwt;
        _logger = logger;
        _msValidator = msValidator;
    }
    ...
    public async Task<IActionResult> MicrosoftLogin([FromBody] MicrosoftLoginDto dto)
    {
        ...
        try
        {
            principal = await _msValidator.ValidateTokenAsync(dto.IdToken);
        }
        catch (Exception ex)
        {
        ...
        if (string.IsNullOrEmpty(email))
            return BadRequest(new { message = "Microsoft account has no email claim." });

        // Optional: restrict to university domain
        // if (!email.EndsWith("@mandela.ac.za", StringComparison.OrdinalIgnoreCase))
        //     return Unauthorized(new { message = "Please use your university Microsoft account." });

        var cleanUsername = email.Split('@')[0];

        _logger.LogInformation("=== MICROSOFT LOGIN ATTEMPT: {Username} ===", cleanUsername);

        var user = await _db.Users.FirstOrDefaultAsync(u =>
            u.Email == email || u.ADUsername == cleanUsername);
        ...
        if (user == null)
        {
            user = new User
            {
                ADUsername = cleanUsername,
                Email = email,
                FirstName = name?.Split(' ').FirstOrDefault() ?? cleanUsername,
            ...
            _db.Users.Add(user);
            await _db.SaveChangesAsync();
            _logger.LogInformation("New user from Microsoft login: {Username}", cleanUsername);
        }

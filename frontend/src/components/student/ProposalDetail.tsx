...
export default function ProposalDetail() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // No evaluations yet, or results endpoint 404s — that's fine,
        // the proposal itself still loaded above.
        setResults(null);
      } finally {
        setLoading(false);
      }
  ...
  const scoreFields: Array<[string, keyof EvaluationResults['evaluations'][number]]> = [
            ...
            <div key={ev.rubricID} style={{ marginBottom: 24, paddingBottom: 24, borderBottom: idx < results.evaluations.length - 1 ? '2px solid var(--border)' : 'none' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
                {scoreFields.map(([label, field]) => {
                  const score = ev[field] as number;
                  return (

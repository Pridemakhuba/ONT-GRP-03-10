...
import type { User, UserRole, ApiErrorResponse } from '../../types';

const ROLES: UserRole[] = ['Student', 'Supervisor', 'Evaluator', 'Admin'];
...
export default function UserManagement() {
    ...
    const [studentForm, setStudentForm] = useState<StudentForm>({ studentNumber: '', program: '' });
    const [supervisorForm, setSupervisorForm] = useState<SupervisorForm>({ expertise: '' });
    ...
    async function saveRole(userId: number) {
        ...
        try {
            ...
            toast.error(apiErr.response?.data?.message || 'Update failed');
    ...
    async function createStudentProfile() {
        ...
        try {
            ...
            const apiErr = err as ApiErrorLike;
            toast.error(apiErr.response?.data?.message || 'Failed to create student profile');
        }
    }
    ...
    async function createSupervisorProfile() {
        ...
        try {
            ...
            const apiErr = err as ApiErrorLike;
            toast.error(apiErr.response?.data?.message || 'Failed to create supervisor profile');
        }
    }
            ...
            <div className="card mb-2">
                <form onSubmit={handleSearch} style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    ...
                    <div className="form-group" style={{ minWidth: 160, marginBottom: 0 }}>
                        ...
                        <select className="form-control" value={filterRole} onChange={e => setFilterRole(e.target.value as UserRole | '')}>
                                    ...
                                    <tbody>
                                        {displayed.map(u => (
                                            <tr key={u.userID}>
                                                ...
                                                <td>
                                                    <span style={{ fontSize: 12, color: u.isActive ? 'green' : 'red' }}>
                                                ...
                                                <td>
                                                    <div style={{ display: 'flex', gap: 6 }}>
                                                        {editing === u.userID ? (
                                                            <>
                                                                <button onClick={() => saveRole(u.userID)} className="btn btn-sm btn-primary" disabled={saving}>
                                                                    {saving ? '...' : '✅'}
                                                                </button>
                                                            ...
                                                            <>
                                                                <button onClick={() => startEdit(u)} className="btn btn-sm btn-ghost" title="Edit Role">✏️</button>
                                                                {u.role === 'Student' && (
                                                                    <button
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
                    <div className="card" style={{ width: 460, maxWidth: '95vw' }}>
                        ...
                        <div className="alert alert-info">
                            Creating student profile for UserID: <strong>{selectedUserId}</strong>
                        </div>
                        <div className="form-group">
                            <label className="form-label">Student Number *</label>
                            <input className="form-control" placeholder="e.g. 220012345"
                                value={studentForm.studentNumber}
                                onChange={e => setStudentForm(p => ({ ...p, studentNumber: e.target.value }))} />
                        </div>
                        <div className="form-group">
                            <label className="form-label">Program *</label>
                            <input className="form-control" placeholder="e.g. MTech Computer Science"
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
                    <div className="card" style={{ width: 460, maxWidth: '95vw' }}>
                        ...
                        <div className="form-group">
                            ...
                            <input className="form-control" placeholder="e.g. Machine Learning, Software Engineering"

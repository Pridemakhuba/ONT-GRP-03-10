...
import type { Proposal, Supervisor, ApiErrorResponse } from '../../types';
...
export default function AssignEvaluators() {
  ...
  const [allSups, setAllSups]     = useState<Supervisor[]>([]);
  ...
  async function handleAssign() {
    ...
    setSaving(true);
    try {
      await proposalsApi.assignEvaluators(proposalId, { proposalID: parseInt(proposalId, 10), evaluatorIDs: selected });
      toast.success('Evaluators assigned! They have been notified by email.');
      navigate('/supervisor/dashboard');
    } catch (err) {
      const apiErr = err as ApiErrorLike;
      toast.error(apiErr.response?.data?.message || 'Assignment failed');
